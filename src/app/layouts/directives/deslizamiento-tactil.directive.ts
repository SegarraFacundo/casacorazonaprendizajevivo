...
@Directive({
  selector: '[deslizamientoTactil]'
})
export class DeslizamientoTactilDirective {

  @Output() derechaAIzquierda = new EventEmitter<void>();
  @Output() izquierdaADerecha = new EventEmitter<void>();
  ...
  swipe(e: TouchEvent, when: string): void {
      ...
      if (when === 'start') {
          ...
          if (duration < 1000 //
              ...
              if (swipeDir === 'next') {
                  this.derechaAIzquierda.emit();
              } else {
                  this.izquierdaADerecha.emit();
              }
          }
      }
  }
